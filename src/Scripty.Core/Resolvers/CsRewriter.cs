namespace Scripty.Core.Resolvers
{
    ...
    public class CsRewriter
    {
        ...
        public static RewrittenAssembly CreateRewriteFileAsAssembly(string rewriteCandidateFilePath)
        {
            ...
            var listOfUsings = GetListOfNamespaces(csExtraction.Namespaces, csExtraction.Namespaces, referencedAssemblies);
        ...
        public static CsExtraction ExtractCompilationDetailFromClassFile(string rewriteCandidateFilePath)
        {
            ...
            var declaredButRemovedNamespaces = new List<string>();
            ...
            foreach (var mcuNamespace in mcuNamespaces)
            {
                ...
                declaredButRemovedNamespaces.Add(namespaceDeclarationSyntax.Name.ToFullString());
                ...
                foreach (var member in namespaceDeclarationSyntax.Members)
                {
                    ...
                    foreach (var classDecl in classDeclarations)
                    {
                        ...
                        foreach (var u in usingsForThisCompilationUnit)
                        {
                            ...
                            // why did MS put Add and With members on an object that is supposed to be immutable?
            ...
            var allNamespaces = GetListOfNamespaces(declaredButRemovedNamespaces, allUsingsAcrossCompilationUnit.Select(u => u.Name.ToString()), metadataReferences: references);
        ...
        private static List<string> GetListOfNamespaces(IEnumerable<string> namespacesToExplode,
            IEnumerable<string> namespacesToUseVerbatim = null, IEnumerable<Assembly> assemblies = null,
            IEnumerable<MetadataReference> metadataReferences = null)
        {
            ...
            //foreach (var removedNamespace in namespacesToExplode)
            //{
            //    var explodedNamespaces = BuildStackedNamespacePaths(removedNamespace);
            //   //dont add them if they arent in the assemblies

            //    listOfUsings.AddRange(explodedNamespaces); 
            //}
